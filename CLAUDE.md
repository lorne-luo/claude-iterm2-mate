...
- **Actions/ItermBgColorAction** — sets the pane background by spawning `set-pane-bg.py <uuid> <RRGGBB>` (iTerm2 Python API) through `PythonInterpreter`. Gated on the script *and* the interpreter being present. Both `arguments(...)` overloads are pure/tested. The bundled script also accepts `default` in place of the hex (restores the underlying profile background via `Profile.original_guid`); nothing in the app sends it today.
