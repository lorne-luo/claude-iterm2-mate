...
- **Scripts/ScriptInstaller + Scripts/PythonInterpreter** — the Python-script counterpart of `HookInstaller`, deliberately **decoupled** from it: pane coloring and click-to-jump are orthogonal to the hook opt-in, so `AppDelegate` publishes `iterm-focus-pane.py` / `set-pane-bg.py` to App Support **unconditionally** on launch (off-main, idempotent, same temp + `replaceItemAt` atomic swap; failure only logs). `ScriptInstaller` writes nothing outside our own App Support directory — never `~/.claude/`. `PythonInterpreter.resolve` finds a Python that can `import iterm2` by reading the **shebang of the `it2` launcher** (it2 is itself a Python script, and `uv tool install it2` is already a documented prerequisite), following the `~/.local/bin/it2` symlink first, falling back to a sibling `python`. `parseShebang` deliberately **rejects `#!/usr/bin/env python3`**: the system Python has no `iterm2` (verified `ModuleNotFoundError`), so accepting an indirect shebang would fail silently at spawn. Resolved per call, never persisted — rewriting a script's shebang at install time would freeze the path and break when the user reinstalls `it2` onto a different Python.
- **Dependencies/DependencyReport** — which external prerequisites are unmet, so the app says so instead of looking healthy while doing nothing. `evaluate(itermInstalled:it2Found:hookInstalled:hasReceivedEvent:)` is pure and holds all the logic (the only test seam); `current()` is the sole system-touching shell. iTerm2 is judged by `NSWorkspace.urlForApplication(withBundleIdentifier: "com.googlecode.iterm2")` — **not** by `ItermSessionLookup`, whose `is running` guard yields an empty `Set` for both "not installed" and "no panes open". `it2` reuses `ItermFocusAction.resolveIt2()`. Delivery is judged by **symptom, not cause**: hook installed but `AppSettings.hasReceivedEvent` still false. `node` is deliberately never probed — a GUI-launched app has only launchd's default `PATH` (`launchctl getenv PATH` is empty) while hooks inherit Claude Code's shell `PATH`, so probing would be a guaranteed false negative; the symptom check also covers `PATH` problems, an un-restarted Claude Code, and socket permissions. Delivery is **not** reported when the hook is absent — that is "not opted in", not "broken".
