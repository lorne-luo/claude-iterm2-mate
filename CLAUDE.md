...
(SessionEnd `reason` ∈ {`exit`, `prompt_input_exit`} — never `clear`, which keeps Claude alive and
is followed by a SessionStart) the pane background is reset to its profile default and **both**
