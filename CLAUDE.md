...
bundled `set-pane-bg.py` (iTerm2 Python API — a per-session profile override that never
...
- **AppSettings** — `UserDefaults`-backed toggles mirrored in the menu bar: `showNonIterm`, `colorPanes`, `showTabStrip`, `playSound` (all default true). `ReminderCoordinator` reads them through injected closures so its logic stays testable. Also holds `hasReceivedEvent`, a one-way latch set by `ReminderCoordinator.onDidReceiveEvent` — diagnostic state, not a preference, so it is deliberately absent from the menu.
...
- **Actions/ItermFocusAction** — jumps to the pane. Maximize-on-click (menu toggle, `UserDefaults`) chooses between the bundled `iterm-focus-pane.py` (focus + maximize, run through `PythonInterpreter`) and the `it2` CLI (`app activate` + `session focus`, focus only). Also exposes `resolveIt2()` / `it2Process(...)` reused by the other `it2` actions. `scriptAvailable` requires **both** the script file and a resolvable interpreter — since the interpreter comes from the `it2` venv, no `it2` means no script path either, so `canFocus` is effectively `it2Available`. `plan()` and `launch(interpreter:scriptPath:sessionUUID:)` are pure/tested.
...
- **Actions/ItermBgColorAction** — sets the pane background by spawning `set-pane-bg.py <uuid> <RRGGBB>` (iTerm2 Python API) through `PythonInterpreter`. `reset(sessionUUID:)` spawns the same script with the `default` sentinel instead of a hex, which restores the pane's underlying profile background (`Profile.original_guid` → `Profile.async_get` → `LocalWriteOnlyProfile`; iTerm2 2.20 has no "drop the override" call, so the override is re-set to the base value). An older *installed* script cannot parse the sentinel as hex: it raises an uncaught ValueError and prints a traceback to the app's inherited stderr, but the pane outcome is still correct (background unchanged, keeps its project color) — degraded, never broken. Not to be confused with `/color default`, the hue `ReminderPalette` excludes. Gated on the script *and* the interpreter being present; both `apply` and `reset` go through the interpreter, never the script's decorative shebang. All four `arguments(...)` / `resetArguments(...)` builders are pure/tested.
...
- **Scripts/ScriptInstaller + Scripts/PythonInterpreter** — the Python-script counterpart of `HookInstaller`, deliberately **decoupled** from it: pane coloring and click-to-jump are orthogonal to the hook opt-in, so `AppDelegate` publishes `iterm-focus-pane.py` / `set-pane-bg.py` to App Support **unconditionally** on launch (idempotent, same temp + `replaceItemAt` atomic swap; failure only logs). It runs **synchronously and before `server.start()`** — unlike the hook refresh, which stays off-main. That ordering is load-bearing: the server accepts payloads the instant it is up, and `colorPaneIfNeeded` writes `coloredSessions[uuid]` *before* firing the action, so a `session_start` that lands while the scripts are still being copied marks the pane done and never retries it for the life of the process. Two ~3 KB copies on the main thread beat a permanently uncolored pane. `ScriptInstaller` writes nothing outside our own App Support directory — never `~/.claude/`. `PythonInterpreter.resolve` finds a Python that can `import iterm2` by reading the **shebang of the `it2` launcher** (it2 is itself a Python script, and `uv tool install it2` is already a documented prerequisite), following the `~/.local/bin/it2` symlink first, falling back to a sibling `python`. `parseShebang` deliberately **rejects `#!/usr/bin/env python3`**: the system Python has no `iterm2` (verified `ModuleNotFoundError`), so accepting an indirect shebang would fail silently at spawn. Resolved per call, never persisted — rewriting a script's shebang at install time would freeze the path and break when the user reinstalls `it2` onto a different Python.
- **Dependencies/DependencyReport (+ ItermPythonAPIProbe, AutomationPermission, UvLocator, LaunchPresentation, Fix)** — which external prerequisites are unmet, so the app says so instead of looking healthy while doing nothing. `evaluate(itermInstalled:it2Usable:uvInstalled:pythonAPI:automation:hookInstalled:hasReceivedEvent:)` is pure and holds all the logic; `current()` is the sole system-touching shell. Six dependencies in declaration = presentation order: `iterm2, it2, uv, pythonAPI, automation, delivery`. Each carries a `severity` (**blocking** = iterm2/it2/pythonAPI/automation, **degraded** = uv/delivery) and rendering copy (`title`/`impact`/`okDetail`/`unknownReason`/`fix`/`menuTitle`/`toastLine`).
  - **Three-valued state** (`DependencyState.ok/.missing/.unknown`), not a Bool. `.unknown` = "we could not check", and it is listed in `all` (a grey `?` row in the Setup window) but deliberately **never** enters `missing`/`blocking`, the menu warning rows, or the launch toast. Automation is unknowable while iTerm2 is not running — precisely the state of a login-item launch — so a Bool would have to lie in one direction or the other.
  - **A row that cannot mean anything is omitted, not faked**: `uv` only when `it2` is unusable (it is purely how `it2` is installed), `pythonAPI`/`automation` only when iTerm2 is installed, `delivery` only when the hook is installed.
  - `missing` / `hasAnyMissing` / `menuTitle` / `toastLine` keep their old semantics and signatures, so the menu's warning-row rendering needed no change (it now simply has more rows to draw); `blocking` / `hasBlocking` / `all` are the additions. `evaluate`'s signature *did* widen, which is a breaking change for its tests by design — it is the seam.
  - iTerm2 is judged by `NSWorkspace.urlForApplication(withBundleIdentifier: "com.googlecode.iterm2")` — **not** by `ItermSessionLookup`, whose `is running` guard yields an empty `Set` for both "not installed" and "no panes open". `it2` counts as usable only when `resolveIt2()` **and** `PythonInterpreter.resolve` both succeed. **But that is still not a capability** — see the "it2 on disk ≠ able to reach iTerm2" gotcha.
  - `ItermPythonAPIProbe.state(rawEnableAPIServer:socketExists:customPrefsFolder:)` reads `EnableAPIServer` out of the `com.googlecode.iterm2` domain via CFPreferences (no subprocess, never launches iTerm2, safe to poll). A live `~/Library/Application Support/iTerm2/private/socket` is an **OR fallback**, never the primary check: it proves the API server is up whatever the plist says (iTerm2 flushes prefs on its own schedule, so without it the row could stay red long after the user ticks the box), but it is equally absent when iTerm2 simply is not running, so on its own it would read "disabled". A custom prefs folder → `.unknown`.
  - `AutomationPermission.state(for:)` maps `AEDeterminePermissionToAutomateTarget`'s return codes: `0` → ok, `-1743` (denied) and `-1744` (never asked) → missing, `-600` → **unknown**. `0` and `-600` were measured against a compiled probe (prd F7); the two consent codes are Apple's documented constants, since observing them means destroying a real TCC grant. `check(askUserIfNeeded: false)` is the silent query used by `current()`; `true` puts up macOS's own consent sheet and blocks, so only the Setup window's Grant… button calls it, off-main.
  - `UvLocator.candidates` mirrors `ItermFocusAction.it2Candidates` — absolute paths only, never a name lookup.
  - Delivery is judged by **symptom, not cause**: hook installed but `AppSettings.hasReceivedEvent` still false. `node` is deliberately never probed — a GUI-launched app has only launchd's default `PATH` while hooks inherit Claude Code's shell `PATH`, so probing would be a guaranteed false negative; the symptom check also covers `PATH` problems, an un-restarted Claude Code, and socket permissions. Delivery is **not** reported when the hook is absent — that is "not opted in", not "broken" — and it is **degraded**, since it is false by construction for anyone who just installed the hook.
  - `LaunchPresentation.decide(report:hookInstalled:suppressed:) -> .setupWindow | .infoToast | .none` is the launch escalation, pulled out as a pure function so the rules are unit-tested instead of manually re-verified by restarting the app under hand-broken dependencies. A missing hook counts toward `.setupWindow` even though it is not a `Dependency` (the window is where Install Hook lives), and `.unknown` rows reach neither branch.
- **Setup/** — the prerequisite checklist window, opened from the menu's permanent `Setup…` item and automatically at launch by `LaunchPresentation`. `SetupRow.rows(report:hookInstalled:)` is pure and decides everything (which rows, order, state dot, subtitle, which button); `SetupView` is only a renderer; `SetupFix` performs the action; `SetupWindowController` owns the window. Notes:
  - **The hook is a row but not a `Dependency`** — folding it into that enum would push "you have not opted in yet" into the menu warnings and the launch toast, where it reads as a fault. It is appended last by `SetupRow.rows` and counted separately by `LaunchPresentation`.
  - **`.unknown` rows re-route their fix to `Open iTerm2`** — both unknowable cases are answered by iTerm2 running (macOS can then reply about Automation; the API socket settles the Python API). The row's own fix would be wrong: `Grant…` cannot ask about a process that is not there.
  - **Two show paths, deliberately different (D6)**: `showAtLaunch()` = `orderFrontRegardless()` with no activation, so it never steals focus from a terminal the user is typing in; `showFromMenu()` = `NSApp.activate` + `makeKeyAndOrderFront`, because the user asked. The launch path leaves the window non-key, hence `FirstMouseHostingView` — otherwise the first click on a button is spent becoming key.
  - **The only real `NSWindow` in the app** (`.titled, .closable`) — `PanelFactory` is the recipe for borderless overlays. It still takes that recipe's `collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary]` + `.floating`: a plain window is pinned to the Space it was created on, and the target user very likely has iTerm2 full-screen on another one, so combined with a non-activating order-front it would simply never be seen.
  - **2 s poll while visible, invalidated on close** — the launch path never activates the app, so `didBecomeActive` never fires and "tick the box in iTerm2 and watch it turn green" would otherwise need a Recheck click. Poll and button share one `reload()`.
  - `AppSettings.suppressSetupAtLaunch` suppresses the **popup only**; the menu-bar triangle and warning rows are unaffected. Automation's `Grant…` becomes `Open Privacy Settings` once macOS reports `-1743`, since it will not show the sheet a second time.
- **MenuBar/MenuBarController** — `NSMenuDelegate`; rebuilds on `menuNeedsUpdate` so the hook light + toggles reflect live state. `menu.autoenablesItems = false` is required — otherwise AppKit re-enables items by target and the "disabled until installed" state silently breaks. Takes a `() -> DependencyReport` closure, **never a cached `Bool`**: the old `let focusAvailable` was frozen at construction, so installing `it2` while the app ran left the warning up until a restart. `menuNeedsUpdate` therefore refreshes the icon as well as the rows. `showDependencyToastIfNeeded()` keeps its name and its single `AppDelegate` call site but now switches on `LaunchPresentation.decide`: the Setup window when something blocking is unmet or the hook is absent, otherwise the existing `InfoToastPanel` summary (no "already shown" flag — it stops on its own once they are satisfied; it must pass a longer `duration`, since the default is 4 s and a multi-line list needs more), otherwise nothing. That toast is **not interactive** (`canBecomeKey: false`), so its last line points at the menu's `Setup…` rather than pretending to be a link. `Setup…` is the permanent first menu item and is always `isEnabled = true` — gating it on the hook would lock the user out of the one screen that installs the hook.

### Bundled resources (`Sources/ClaudeItermMate/Resources/`)

All four are loaded at runtime via `Bundle.module` and declared in `Package.swift` `resources` (there is no `exclude` list):

- **Hook scripts** — `mate-notify.js`, `mate-session-start.js`. `HookInstaller` copies them to App Support and registers them in `settings.json`.
- **iTerm2 Python API scripts** — `iterm-focus-pane.py` (spawned by `ItermFocusAction`), `set-pane-bg.py` (spawned by `ItermBgColorAction`). `ScriptInstaller` publishes them to the same App Support directory, and they are run as `<interpreter> <script> <args…>` — **their `#!/usr/bin/env python3` shebang is decorative** and would resolve to a Python without `iterm2`. `ScriptInstallerTests` asserts the shebang and the absence of any `/Users/` path, so a stale machine-local copy cannot be committed silently.

`open-vscode.py` / `open-zed.py` (iTerm2 "Run Command" hotkey scripts that open the session's cwd in an editor) are deliberately **not** part of this repo: they have zero coupling to the app, and the app cannot bind an iTerm2 hotkey on the user's behalf, so bundling them would solve distribution without solving installation. They stay machine-local in `~/.claude/scripts/`.

## Constraints

- macOS 14+, Swift tools 5.9, **no external Swift dependencies**, POSIX sockets only. Bundled hook scripts are self-contained Node (built-ins only); the bundled `.py` scripts use the iTerm2 Python API and therefore need the `it2` venv's interpreter (never vendor `iterm2` + `websockets` + `protobuf` into the bundle — ~5.4 MB with `.so` files pinned to one CPython ABI).
...
- **Never store an `Iterm*Action`; build it at the point of use**: all four resolve `it2` (and its interpreter) in `init`, so a stored copy freezes that answer at launch. `AppDelegate` used to hold them as `private let` and capture them into closures — a user who installed `it2` while the app ran then saw the menu warning clear (`DependencyReport` re-resolves every time) while every click, pane color and answer stayed dead, which is worse than the original frozen warning because the UI actively claims health. Capturing a struct into a closure has the same effect as storing it. They are cheap — a few `isExecutableFile` stats and one 512-byte read — and everything they gate spawns a `Process` anyway. Same rule applies to anything else whose availability the user can change at runtime.
- **`it2` on disk ≠ able to reach iTerm2**: iTerm2's Python API ships **off** (`defaults read com.googlecode.iterm2` → `EnableAPIServer = 0` on a fresh machine, and `~/Library/Application Support/iTerm2/private/` does not even exist until it is enabled). With it off, `it2 session list` fails outright ("Not running inside iTerm2 or Python API not enabled") — so jump, pane background, `/color` and question answers are all dead — while `resolveIt2()` and `PythonInterpreter.resolve` both succeed and the menu bar shows a healthy `bell.badge`. That was the app's state on the author's own machine. Availability of a binary is a proxy; a capability check has to include `ItermPythonAPIProbe`. Note the AppleScript path (`ItermSessionLookup`) is independent and keeps working — which is why "some things work" is not evidence.
- **`AEDeterminePermissionToAutomateTarget` cannot distinguish "not running" from "not installed"**: both return `-600` (`procNotFound`) — verified by compiling a probe and running it against iTerm2 (`0`), an installed-but-not-running app (`-600`) and a nonexistent bundle id (`-600`). Harmless here only because `DependencyReport` asks about Automation solely when `NSWorkspace` has already confirmed iTerm2 is installed, so `-600` can only mean "not running" → `.unknown`. Don't reuse the code as an installed-ness check. Note the app's actual AppleScript goes through a `Process(/usr/bin/osascript)` child; this API asks about *this* process's TCC record, on the assumption that TCC's responsible process is still the app. If that ever proves wrong, the fallback is to stop discarding `osascript`'s stderr (`nullDevice` today) and look for `-1743`.
- **A GUI-launched app cannot see the user's toolchain**: `launchctl getenv PATH` is empty, so an app started from Finder / Launch-at-Login inherits only `/usr/bin:/bin:/usr/sbin:/sbin`. Measured on this machine: `node` lives in `~/.nvm/versions/node/*/bin` and `/opt/homebrew/bin`, `uv` in `~/.local/bin`, `brew` in `/opt/homebrew/bin` — **none** on that default `PATH`, while the Node hooks see all of them because Claude Code launches them from a shell. So never probe a user tool by name or bare `Process` lookup: either resolve absolute candidate paths (`ItermFocusAction.resolveIt2()`, `PythonInterpreter`) or detect the *symptom* instead (`DependencyReport`'s delivery check). A naive `which node` is a guaranteed false negative. Launching via `open` from a terminal hides this — the app then inherits the shell's `PATH`, so verify from Finder or after a login.
